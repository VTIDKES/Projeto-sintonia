    ...
    try:
        ...
        for bloco in blocos_diagrama:
            ...
            if bloco['tipo'] == 'Transferência':
                ...
                # Aproximação de Pade para atraso
                tau = float(config.get('tf', '1').replace('e^{ -', '').replace('s}', ''))
                if '=' in str(tau):
                    tau = float(tau.split('=')[1])
...
def main():
    ...
    if st.session_state.modo_editor == 'visual':
        ...
        with col2:
            if st.button("📥 Importar Diagrama", use_container_width=True):
        ...
        try:
            if 'message' in st.session_state:
                ...
                if msg['type'] == 'salvar_diagrama':
                    st.session_state.diagrama_blocos['blocos'] = msg['blocos']
                    st.session_state.diagrama_blocos['conexoes'] = msg['conexoes']
                    st.session_state.bloco_contador = msg['contador']
                    st.success("Diagrama salvo automaticamente!")
        except:
            pass
