...
def estimar_tempo_final_simulacao(tf):
    ...
    return float(np.clip(tempo_final, a_min=10, a_max=500))
...
def criar_diagrama_blocos_html():
    """Cria o editor visual de diagrama de blocos (SEM f-string quebrando por chaves do JS)"""
    ...
    contador = str(st.session_state.bloco_contador)
...
<head>
    <style>
        ...
        .bloco-tipo { font-size: 10px; opacity: 0.8; margin-bottom: 5px; }
        .bloco-nome { font-weight: bold; font-size: 14px; margin-bottom: 5px; }
        .bloco-tf {
            font-size: 11px;
        ...
        .porta:hover { background: #45a049; transform: scale(1.3); }
...
<body>
    ...
    <script>
        let blocos = __BLOCOS_DATA__;
        let conexoes = __CONEXOES_DATA__;
        let blocoIdCounter = __CONTADOR__;
        ...
        function adicionarBlocoSomador() { adicionarBloco("Somador", {nome: "Σ" + blocoIdCounter}); }
        ...
        // ✅ CORRIGIDO: sem e^{...} que quebra f-string; agora salva "valor" do atraso
        ...
        function adicionarBlocoFeedback() { adicionarBloco("Feedback", {nome: "H" + blocoIdCounter}); }
        ...
        function clickPorta(e) {
            ...
            if (!portaSelecionada) {
                ...
                if (portaSelecionada.tipo === "saida" && tipoPorta === "entrada") {
                    ...
                    conexoes.push({origem: blocoId, destino: portaSelecionada.blocoId});
                    redesenharConexoes();
                } else {
        ...
        function limparDiagrama() {
            if (confirm("Deseja limpar todo o diagrama?")) {
                ...
                document.getElementById("canvas-container").innerHTML =
                    '<svg id="conexoes-svg"></svg><div id="info-panel"><strong>📊 Instruções:</strong><div>• Clique nos botões para adicionar</div><div>• Arraste blocos para mover</div><div>• Clique nas portas verdes para conectar</div><div>• Clique no bloco para selecionar</div></div>';
                salvarEstado();
            }
        }
        ...
        blocos.forEach(blocoData => {
            ...
            let nomeDisplay = (blocoData.config && blocoData.config.nome) ? blocoData.config.nome : blocoData.tipo;
            let tfDisplay = (blocoData.config && blocoData.config.tf) ? blocoData.config.tf : "";
...
'''
    ...
    html = html_template
    html = html.replace("__BLOCOS_DATA__", blocos_data)
    html = html.replace("__CONEXOES_DATA__", conexoes_data)
    html = html.replace("__CONTADOR__", contador)
    return html
    ...
    try:
        ...
        for bloco in blocos_diagrama:
            ...
            if bloco['tipo'] == 'Transferência':
                ...
                # ✅ agora usa config['valor'] (tau) corretamente
                tau = float(config.get('valor', 1.0))
                # aproximação simples (1 / (tau*s + 1)) só para não quebrar:
                tfs[bloco_id] = TransferFunction([1], [tau, 1])
            elif bloco['tipo'] == 'Somador':
                tfs[bloco_id] = TransferFunction([1], [1])
            elif bloco['tipo'] == 'Feedback':
                tfs[bloco_id] = TransferFunction([1], [1])
...
def main():
    ...
    st.session_state.modo_editor = 'visual' if modo == 'Editor Visual (Xcos)' else 'classico'
    ...
    if st.session_state.modo_editor == 'visual':
        ...
        with col1:
            if st.button("⚡ Processar Diagrama", type="primary", use_container_width=True):
                ...
                if sistema:
                    ...
                    tab1, tab2, tab3, tab4 = st.tabs(["📈 Resposta Temporal", "📊 Bode", "🎯 Polos e Zeros", "🔄 LGR/Nyquist"])
                    ...
                    with tab4:
                        col_lgr, col_nyquist = st.columns(2)
                        with col_lgr:
        ...
        with col2:
            uploaded_file = st.file_uploader("📥 Importar Diagrama (JSON)", type="json", label_visibility="visible")
            if uploaded_file is not None:
                try:
                    data = json.load(uploaded_file)
                    st.session_state.diagrama_blocos = data
        ...
        with btn_col1:
            if st.button("🔍 Calcular Erro Estacionário"):
                try:
                    ...
                    tipo_sys, Kp, Kv, Ka = constantes_de_erro(G)

                    df = pd.DataFrame([{"Tipo": tipo_sys, "Kp": Kp, "Kv": Kv, "Ka": Ka}])
